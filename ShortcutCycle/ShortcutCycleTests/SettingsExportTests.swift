...
final class SettingsExportTests: XCTestCase {
    ...
    func testSettingsChangeTriggersAutoBackup() async throws {
        let userDefaults = UserDefaults(suiteName: "TestAutoBackupSettings")!
        userDefaults.removePersistentDomain(forName: "TestAutoBackupSettings")
        let store = GroupStore(userDefaults: userDefaults)
        
        // Ensure no initial backup is pending
        store.flushPendingBackup()
        
        // Change a setting observable by GroupStore
        userDefaults.set("dark", forKey: "appTheme")
        
        // Allow Task { @MainActor ... } in observer to run
        try await Task.sleep(nanoseconds: 100_000_000) // 0.1s
        
        // Flush the pending backup
        store.flushPendingBackup()
        ...
        if let files = try? fileManager.contentsOfDirectory(at: backupDir, includingPropertiesForKeys: nil) {
            let backupFiles = files.filter { $0.lastPathComponent.hasPrefix("backup ") && $0.pathExtension == "json" }
            XCTAssertFalse(backupFiles.isEmpty, "Backup should be created after changing 'appTheme'")
            ...
            XCTFail("Could not list backup directory")
        }
        
        userDefaults.removePersistentDomain(forName: "TestAutoBackupSettings")
    }
    func testIdenticalSettingsUpdateDoesNotTriggerBackup() async throws {
        let userDefaults = UserDefaults(suiteName: "TestAutoBackupRedundant")!
        userDefaults.removePersistentDomain(forName: "TestAutoBackupRedundant")
        let store = GroupStore(userDefaults: userDefaults)
        
        // Initial set (triggers immediate backup)
        userDefaults.set("dark", forKey: "appTheme")
        try await Task.sleep(nanoseconds: 100_000_000)
        store.flushPendingBackup()
        
        // Clear backup directory to verify next step
        let fileManager = FileManager.default
        let appSupport = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let backupDir = appSupport.appendingPathComponent("ShortcutCycle-Test", isDirectory: true)
        if let files = try? fileManager.contentsOfDirectory(at: backupDir, includingPropertiesForKeys: nil) {
        ...
        // flush should do nothing if no backup pending (because logic should ignore redundant update)
        store.flushPendingBackup()
        
        // Verify NO backup created
        let files = try? fileManager.contentsOfDirectory(at: backupDir, includingPropertiesForKeys: nil)
        let backupFiles = files?.filter { $0.lastPathComponent.hasPrefix("backup ") && $0.pathExtension == "json" } ?? []
        XCTAssertTrue(backupFiles.isEmpty, "Redundant update should not trigger backup")
        
        userDefaults.removePersistentDomain(forName: "TestAutoBackupRedundant")
    }
    func testStartQuitWithNoChangesDoesNotCreateBackup() async throws {
        let userDefaults = UserDefaults(suiteName: "TestAutoBackupStartQuit")!
        userDefaults.removePersistentDomain(forName: "TestAutoBackupStartQuit")
        
        // 1. Initial run: Create data and backup
        var store = GroupStore(userDefaults: userDefaults)
        _ = store.addGroup(name: "Initial Group")
        store.flushPendingBackup() 
        ...
        guard let files1 = try? fileManager.contentsOfDirectory(at: backupDir, includingPropertiesForKeys: nil) else {
            XCTFail("Backup dir not found")
            return
        }
        let count1 = files1.filter { $0.lastPathComponent.hasPrefix("backup ") }.count
        XCTAssertEqual(count1, 1, "Should have 1 backup initially")
        
        // 2. Simulate App Restart (New Store)
        store = GroupStore(userDefaults: userDefaults)
        
        // Force trigger backup (simulate KVO or termination check)
        userDefaults.set("system", forKey: "appTheme") 
        try await Task.sleep(nanoseconds: 100_000_000)
        store.flushPendingBackup()
        
        // 3. Verify NO new backup file
        let files2 = try? fileManager.contentsOfDirectory(at: backupDir, includingPropertiesForKeys: nil)
        let count2 = files2?.filter { $0.lastPathComponent.hasPrefix("backup ") }.count ?? 0
        
        XCTAssertEqual(count2, 1, "Should still have 1 backup if content identical")
