...
final class PressAndHoldTests: XCTestCase {
    ...
    @MainActor
    func testTapToCycleModeRevealsImmediatelyAndAutoFinalizes() async {
        var activatedAppIds: [String] = []
        manager.activatePendingTargetApp = { appId in
            activatedAppIds.append(appId)
        }
        ...
        XCTAssertTrue(manager.isVisible, "Tap-to-cycle should reveal the HUD immediately")
        XCTAssertTrue(manager.isSessionActive, "Tap-to-cycle should keep a visible HUD session open")
        XCTAssertFalse(localMonitorMasks.contains(.flagsChanged), "Tap-to-cycle should not finalize from modifier release monitoring")
        ...
        XCTAssertFalse(manager.isSessionActive, "The idle timer should end the tap-to-cycle session")
        XCTAssertEqual(activatedAppIds, ["com.test.1"], "The idle timer should activate the pending selection once")
    }

    @MainActor
    func testTapToCycleModeRepeatedShortcutAdvancesSelectionAndResetsTimer() async {
        var activatedAppIds: [String] = []
        manager.activatePendingTargetApp = { appId in
            activatedAppIds.append(appId)
        }
        ...
        XCTAssertTrue(manager.isVisible, "A repeated tap should keep the HUD visible")
        XCTAssertEqual(manager.currentSelectedAppId, "com.test.2", "A repeated tap should advance to the supplied next selection")

        timerMock.fireLastNonRepeatingTimer()
        await Task.yield()
        await Task.yield()

        XCTAssertEqual(activatedAppIds, ["com.test.2"], "Auto-finalize should activate the latest tap selection")
    }

    @MainActor
    func testTapToCycleModeEscapeCancelsWithoutActivation() async throws {
        var activateCount = 0
        ...
        manager.scheduleShow(
            items: [HUDAppItem(bundleId: "com.test.cancel", pid: 12, name: "Cancel Target")],
            activeAppId: "com.test.cancel::12",
            modifierFlags: [.option],
            shortcut: "Opt+1",
            activeKey: .a,
            shortcutTriggerMode: .tapToCycle,
        ...
        let keyDownHandler = try XCTUnwrap(latestLocalMonitorHandler(for: .keyDown))
        _ = keyDownHandler(try makeKeyEvent(type: .keyDown, keyCode: 53))
        await Task.yield()
        await Task.yield()

        XCTAssertFalse(manager.isSessionActive, "Escape should dismiss the tap-to-cycle HUD")
        XCTAssertEqual(activateCount, 0, "Escape should cancel without activating the pending target")
        XCTAssertTrue(finalizedAppIds.isEmpty, "Escape should not fire finalize callbacks")
    }
